...
export default async function deleteUserAction() {
    return authAction((await cookies()).get('accessToken'), action);
}
...
async function action(loggedUser: UserJWTPayload) {
    ...
    if (res.length > 0) {
        ...
        cookiesStore.set('refreshToken', 'del', { httpOnly: true, sameSite: 'strict', domain: process.env.DOMAIN, path: '/api/auth/refreshtoken', maxAge: 0 });
