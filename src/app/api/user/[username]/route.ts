import { eq, and, isNotNull, countDistinct } from 'drizzle-orm';
...
const withIsFollowing = <T extends PgSelect>(qb: T, userID: number) => {
...
export async function GET(request: Request, { params }: { params: { username: string } }) {
    ...
    let userQuery = db.select({
        ...
        follower: countDistinct(follower),
        following: countDistinct(following),
        ...(isUserProfile && { is_following: isNotNull(isFollowing) })
    }).from(user)
        .leftJoin(following, eq(following.userID, user.id))
    ...
    if (isUserProfile) {
        userQuery = withIsFollowing(userQuery, userID)
    }
